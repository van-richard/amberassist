...
if __name__ == "__main__":
    ...
    # i = int(sys.argv[1])
    # qm_coords = np.load("qm_coord.npy")[i*1000:(i+1)*1000]
    # mm_coords = np.load("mm_coord.npy")[i*1000:(i+1)*1000]
    # qm_elems = np.loadtxt("qm_elem.txt", dtype=int)
    # mm_charges = np.load("mm_charge.npy")[i*1000:(i+1)*1000]
    # energy, qm_grad, mm_esp, mm_esp_grad  = get_qchem_training_set(qm_coords, qm_elems, mm_coords, mm_charges, -1, 1)
