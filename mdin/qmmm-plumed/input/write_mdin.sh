#!/bin/bash
# Render a single QMMM+PLUMED mdin file into the parent run directory.
...
for v in qmmask qmtheory qmcharge thermostat NSTEPS5 print_freq; do
...
if [[ "$qmtheory" == "EXTERN" ]]; then
  ...
  [[ -f "$qmhub_ini" ]] || { echo "ERROR: missing $qmhub_ini" >&2; exit 1; }
  ln -sf "input/${qmhub_ini}" "${run_dir}/qmhub.ini"
fi
...
qmhubscratch="${QMHUBSCRATCH:-./qmhub}"
...
if [[ "$qmtheory" == "EXTERN" ]]; then
  ...
  /\n'
  QMHUB_BLOCK="${QMHUB_BLOCK/__QMHUBSCRATCH__/$qmhubscratch}"
...

{
  ...
  $0 = repl($0, "__QMHUBSCRATCH__", qmhubscratch)
  print
}
' "$TMPL" > "$OUT"
