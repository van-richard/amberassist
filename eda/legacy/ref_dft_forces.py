...
pstep = __PSTEP__
frames = __ISTEP__ # want 250
num_qm_atoms = 73
num_prot_atoms = 27086

fin = (f"qmhub/qmmm.inp_{i:04d}" for i in range(pstep,frames,2))

qm_index = np.array([59, 52, 51])

qmmm = QMMM(mode="text", driver="sander", cwd=Path("dft_forces__ARRAY__"))
...
np.save("qmmm_energy__ARRAY__", energy)
np.save("qmmm_forces__ARRAY__", forces)
np.save("qmmm_prot_forces__ARRAY__", prot_forces)
np.save("qmmm_near_forces__ARRAY__", near_forces)
np.save("qmmm_far_forces__ARRAY__", far_forces)
np.save("qmmm_mm_esp__ARRAY__", mm_esp_all)
