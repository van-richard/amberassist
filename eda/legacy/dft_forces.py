...
#Iteracting so I can have intermediate files
fin = (f"qmhub/qmmm.inp_{i:04d}" for i in range(pstep,frames,2))

qm_index = np.array([59, 52, 51])

qmmm = QMMM(mode="text", driver="sander", cwd=Path("dft_forces"))
...
np.save("qmmm_energy", energy)
np.save("qmmm_forces", forces)
np.save("qmmm_prot_forces", prot_forces)
np.save("qmmm_near_forces", near_forces)
np.save("qmmm_far_forces", far_forces)
np.save("qmmm_mm_esp", mm_esp_all)
